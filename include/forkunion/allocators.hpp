 ...
 *  memory domain, and the @c replicated_array and @c sharded_array containers spreading a sequence
 *  across domains. Execution - the pools - lives in `flat.hpp` and `distributed.hpp`.
 */
#pragma once
...
inline status_t linux_numa_bind(void *ptr, std::size_t size_bytes, memory_domain_id_t memory_domain_id) noexcept {
    ...
    // ! only demand a `CAP_SYS_NICE` that a sandbox may refuse.
...
/**
 *  @brief Tries allocating and binding uninitialized memory to a specific NUMA @p memory_domain_id.
...
inline void *linux_numa_allocate(std::size_t size_bytes, std::size_t page_size_bytes,
    ...
    // default policy rather than pinned to this domain. Discarding it would fail an allocation that
    // in fact succeeded; `runtime_capabilities()` is where a caller learns placement failed.
 ...
 *
 *  A light-weight, but high-latency BLOB allocator, tied to a specific NUMA node ID: every
 *  allocation is one @c mmap plus @c mbind syscall, aligned to at least a 4 KB page size.
 *
 *  @section allocators_cpp23 C++ 23 Functionality
 *
 *  Whenever possible, the newer @c allocate_at_least API reduces the number of reallocations.
 */
...
struct linux_numa_allocator {
  ...
  public:
    ...
    /**
     *  @brief Allocates memory for at least @p size elements of @c value_type.
     *  @param[in] size The number of elements to allocate.
     *  @param[in] page_size_bytes The size of the memory page to allocate, must be a multiple of
     *      `sizeof(value_type)`.
     *  @return An @c allocation_result holding the pointer and the element count, empty when the
    ...
    /**
     *  @brief Allocates a memory for @p size elements of @c value_type.
     *  @param[in] size The number of elements to allocate.
     *  @param[in] page_size_bytes The size of the memory page to allocate, must be a multiple of
     *      `sizeof(value_type)`.
     *  @return An @c allocation_result holding the pointer and the element count, empty when the
    ...
    allocation_result<value_type *, size_type> allocate_at_least(size_type size) noexcept {
        // Go through all of the typical Linux page sizes, finding the largest one that makes sense
        // and doesn't fail.
        size_type const size_bytes = size * sizeof(value_type);
    ...
    /**
     *  @brief Allocates memory for @p size elements of @c value_type.
     *  @param[in] size The number of elements to allocate.
     *  @return An @c allocation_result holding the pointer and the element count, empty when the
     *      allocation fails or the size is not a multiple of `sizeof(value_type)`.
     */
 ...
 *  @return nullptr if the mapping or any slice binding failed, or the page size is unsupported.
 *
 *  Unlike @c linux_numa_allocate, which places a block on a single node, this reserves @b one
...
inline void *linux_symmetric_allocate(machine_topology_t const &topology, std::size_t stride_bytes,
    ...
    // A single memory domain needs no placement - the one slice already spans the only node.
    if (domains > 1)
        for (std::size_t domain = 0; domain != domains; ++domain) {
            ...
            // Best-effort, as in `linux_numa_allocate`: the slice stays validly mapped even if
            // `mbind` is refused - default placement, not a failed allocation.
...
struct linux_symmetric_allocator {
  ...
  private:
    /** The topology whose domains the mapping stripes across; borrowed, not owned. */
  ...
  public:
    ...
    /** Allocates at least @p size elements @b per domain, page-aligning the per-slice stride. */
    ...
    /** Allocates at least @p size elements per domain, trying the largest huge page that fits. */
...
/**
 *  @brief Points the calling thread's @c domainset at @p memory_domain_id with a PREFER policy.
 *  @return true when the prior domainset landed in @p saved_set / @p saved_policy for restore.
 *
 *  FreeBSD 12 removed @c numa_setaffinity, so there is no per-allocation @c numa_alloc_onnode: the
 *  memory domain is a @b thread policy. The caller sets it, first-touches the region so its pages
...
/**
 *  @brief Tries allocating and placing uninitialized memory on a specific NUMA @p memory_domain_id.
 *  @return A pointer to the allocated memory, or nullptr when the allocation failed or the page
 *      size stays unsupported.
 *  @sa linux_numa_allocate is the Linux counterpart; FreeBSD places by thread policy, not @c mbind.
 *
 *  Larger-than-base page sizes ask for @c MAP_ALIGNED_SUPER - an alignment hint, since FreeBSD's
 *  superpages are transparent and have no @c MAP_HUGETLB-style pool. It is best-effort: a failed
...
inline void *freebsd_domain_allocate(std::size_t size_bytes, std::size_t page_size_bytes,
    ...
    if (result_ptr == MAP_FAILED && huge_requested) {
        // Superpage alignment failed; retry with base pages so the allocator stays total.
        mmap_flags &= ~MAP_ALIGNED_SUPER;
...
struct freebsd_numa_allocator {
  ...
  public:
    ...
    value_type *allocate(size_type size) noexcept {
        size_type const size_bytes = size * sizeof(value_type);
        // ! As on Linux, `deallocate(p, n)` only knows `n` - exact-multiple huge pages only.
        if (size_bytes >= (2u * page_size_1g_k) && size_bytes % page_size_1g_k == 0)
            if (auto result = allocate(size, page_size_1g_k); result) return result;
        if (size_bytes >= (2u * page_size_2m_k) && size_bytes % page_size_2m_k == 0)
            if (auto result = allocate(size, page_size_2m_k); result) return result;
...
/**
 *  @brief Maps `domains * stride_bytes` and PREFER-touches slice @p d on `domain_ids[d]`.
...
/**
 *  @brief Allocator for a @b symmetric mapping on FreeBSD: one range striped across every domain.
 *  @sa linux_symmetric_allocator is the Linux counterpart, borrowing its @ref machine_topology.
 */
template <typename value_type_ = char>
...
/**
 *  @brief Enables @c SeLockMemoryPrivilege for the process, needed before large-page allocation.
 *  @return true if the privilege is now held by the process token.
 *  @note This only @b enables a privilege the account already holds.
 *
 *  The account must first be granted "Lock pages in memory" - Local Security Policy /
 ...
 *  @param[in] large_pages Request @c MEM_LARGE_PAGES; the size is rounded up to
 *      `GetLargePageMinimum()`.
 ...
 *  @return nullptr if the reservation or any slice commit failed.
 *
 *  The Windows analogue of @c linux_symmetric_allocate: one `VirtualAlloc(MEM_RESERVE)` for the
 *  whole range, then a per-slice `VirtualAllocExNuma(MEM_COMMIT, node)`, so each slice's pages
...
/**
 *  @brief STL-compatible allocator bound to a single memory domain on Windows, backed by
 *      @c VirtualAllocExNuma.
 *  @sa linux_numa_allocator is the Linux counterpart; both satisfy the pool's allocator needs.
 *
...
struct windows_numa_allocator {
  ...
  public:
     ...
     *  @brief Allocates memory for at least @p size elements of @c value_type.
     *  @param[in] size The number of elements to allocate.
     *  @return An @c allocation_result holding the pointer and the element count, empty when
     *      allocation fails outright.
     *  @note Unlike @c linux_numa_allocator, there is no huge-page ladder - @c VirtualAllocExNuma
     *      commits base or large pages per @c large_pages.
     */
    allocation_result<value_type *, size_type> allocate_at_least(size_type size) noexcept {
...
struct portable_aligned_allocator {
  ...
  public:
    ...
    /** Allocates at least @p size elements, rounded up to the page size. */
    allocation_result<value_type *, size_type> allocate_at_least(size_type size) noexcept {
        size_type const page_size_bytes = default_page_size_ ? default_page_size_ : ram_page_size();
        size_type const aligned_size_bytes = round_up_to_multiple(size * sizeof(value_type), page_size_bytes);
    ...
    /** Allocates exactly @p size elements. */
    value_type *allocate(size_type size) noexcept {
...
struct portable_symmetric_allocator {
  ...
  public:
    ...
    /** Allocates one @c default_alignment_k-aligned block of `domains * stride_bytes`, unbound. */
    allocation_type allocate_at_least(size_type size) noexcept {
 ...
 *
 *  For read-shared data - graph adjacency, body positions. The logical length is @b n; the
 *  footprint is `n * memory_domains_count()`. It owns memory and hands out slices - the caller
 *  fills every replica and keeps them coherent. Backed by one symmetric mapping, so replica @c d of
 *  element @c i lives at `base() + d * stride_bytes() + i * sizeof(value_type)`.
 */
...
struct replicated_array {
  ...
  private:
    ...
    /** Logical length @c n, identical in every replica. */
    std::size_t size_ {0};
  ...
  public:
    ...
    /** Frees the mapping. Elements are raw storage, so no destructors run. */
    void reset() noexcept {
    ...
    /** One element of the replica on @p memory_domain. */
    ...
    /** The whole replica living on @p memory_domain. */
...
/**
 *  @brief A domain-local buffer - a sequence partitioned once across every memory domain.
 *  @tparam value_type_ The element type; treated as raw storage - the container runs no
 *      constructors or destructors.
 *  @tparam allocator_type_ A symmetric mapping allocator - one range striped across the domains.
 *
 *  For datasets too large to replicate. The logical length is @b n; the footprint is @c n. Each
 *  domain owns a @b contiguous logical segment of `segment() == ceil(n / memory_domains_count())`
 *  elements, so element @b i lives at `{i / segment(), i % segment()}` - see @c location_of - and a
 *  sequential scan of a domain's shard is sequential in memory. It owns memory and hands out
 *  slices; the caller fills them. Backed by one symmetric mapping whose uniform stride is that
 *  segment, so a shorter trailing shard leaves slots unused.
 */
template <typename value_type_, typename allocator_type_ = symmetric_memory_allocator_t>
struct sharded_array {
  ...
  private:
    ...
    /** Logical length @c n, summed across the shards. */
    std::size_t size_ {0};
  ...
  public:
    ...
    /** How many elements the shard on @p memory_domain holds - a trailing shard may be shorter. */
    ...
    /** The logical index of the element at @p local_index on @p memory_domain - inverse of
     *  @c location_of. */
    ...
    /** The single home of a logical element - freely mutable, unlike a replica. */
    value_type_ &at(memory_domain_index_t memory_domain, std::size_t local_index) noexcept {
        return allocation_.slice(static_cast<std::size_t>(memory_domain))[local_index];
    }
    value_type_ const &at(memory_domain_index_t memory_domain, std::size_t local_index) const noexcept {
    ...
    /** The whole shard living on @p memory_domain. */
    span_type on_memory_domain(memory_domain_index_t memory_domain) noexcept {
