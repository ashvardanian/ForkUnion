 ...
 *  @note Linux's @c clock_nanosleep lets us name the clock; Darwin only has @c nanosleep, whose
...
struct alignas(default_alignment_k) pinned_thread_t {
    ...
    /** The OS thread handle: @c pthread_t on POSIX, @c HANDLE on Windows. */
    std::atomic<native_thread_t> handle {};

    /** The OS thread id: @c gettid on Linux, @c pthread_threadid_np on Apple, and
     *  @c GetCurrentThreadId on Windows. */
    ...
    /**
     *  @brief This thread's private cursor for @c for_n_dynamic; see @ref dynamic_claim.
     ...
     *  @note Fixed to @c std::size_t because @c colocated_pool is not templated on an index width,
     *      unlike @c flat_pool. The narrow-index debug configs, and the cursor's overflow argument,
     *      therefore only ever exercise @c flat_pool.
     */
...
struct colocated_pool {
  ...
  public:
    ...
    /** Replaces the pool's name; only threads spawned after the call pick it up. */
     ...
     *  @return -1 if the thread-pool is not initialized or the memory domain is unknown.
     *  @note This API is @b not synchronized.
     */
     ...
     *  @return 0 if the thread-pool is not initialized or the compute_domain index is unknown.
     *  @note This API is @b not synchronized.
     */
...
#pragma region Core API
    ...
    /** Workers the kernel refused to place; zero on a fully pinned pool. */
     ...
     */
    status_t spawn(compute_domain_t const &domain,
                   caller_exclusivity_t const exclusivity = caller_inclusive_k) noexcept {
        return spawn(domain, domain.logical_cores_count, exclusivity);
    }
     ...
     */
    status_t spawn(compute_domain_t const &domain, thread_index_t const threads,
                   ...
                   index_t const compute_domain_index = 0, [[maybe_unused]] index_t const compute_levels = 1) noexcept {
        ...
        if (status_t const grew = pthreads.resize(threads); failed(grew)) return grew;
        ...
        [[maybe_unused]] status_t const captured = capture_thread_cores(caller_affinity_);
        ...
        for (thread_index_t i = use_caller_thread; i < threads; ++i) {
            ...
            if (!created) {
                ...
                for (thread_index_t j = use_caller_thread; j < i; ++j) {
                    ...
                    [[maybe_unused]] int join_result = ::pthread_join(started, nullptr);
        ...
        if (pin_granularity == pin_to_core_k) {
            for (thread_index_t i = 0; i < pthreads_.size(); ++i) {
                ...
                if (succeeded(pin_thread_to_cores(pin_handle, &cpu, 1))) pthreads_[i].core_id = cpu;
                else ++unpinned_threads_;
            }
        }
        else {
            for (thread_index_t i = 0; i < pthreads_.size(); ++i) {
                native_thread_t const pin_handle = pthreads_[i].handle.load(std::memory_order_relaxed);
                if (failed(pin_thread_to_cores(pin_handle, domain.first_core_id, domain.logical_cores_count)))
                    ++unpinned_threads_;
            }
        }
...
#pragma region Control Flow
    ...
    void terminate() noexcept {
        ...
        for (thread_index_t i = use_caller_thread; i != threads; ++i) {
            ...
            [[maybe_unused]] int const join_result = ::pthread_join(join_handle, &returned_value);
...
#pragma region Advanced
     ...
     *
     *  On @c caller_inclusive_k pools this can only turn @c true once @c unsafe_join contributes
     *  the calling thread's slice, so poll-then-join is reserved for @c caller_exclusive_k pools.
     */
...
#pragma region ComputeDomains Compatibility
     ...
     *  @return 1 constant for compatibility.
     */
     ...
     */
    thread_index_t first_thread([[maybe_unused]] index_t compute_domain_index) const noexcept {
        assert(compute_domain_index == 0 && "Only one compute_domain is supported");
        return first_thread();
    }
    ...
    constexpr thread_index_t thread_local_index(thread_index_t global_thread_index,
                                                [[maybe_unused]] index_t compute_domain_index = 0) const noexcept {
        assert(compute_domain_index == 0 && "Only one compute_domain is supported");
...
#pragma endregion ComputeDomains Compatibility
  ...
  private:
     ...
     *  name parses to @c apple_performance_k, falling back to rank alone, never to @c UTILITY.
     */
 ...
 *  @note Like @c invoke_for_n_dynamic, but determines each thread's compute domain at run time.
 *
 *  @section distributed_scheduling Scheduling Logic
 *
 *  The same protocol as @c invoke_for_n_dynamic, applied at two levels. Let's say we receive N
 *  tasks for T threads across C compute_domains. Each compute_domain takes (N/C) tasks, reserves
 *  one trailing static prong per local thread, and splits the rest into one contiguous slice per
 *  local thread - so every thread drains its own slice with an @b uncontended @c fetch_add on a
 *  cursor line nobody else touches. A drained thread first helps its same-domain neighbours,
 *  walking them in a coprime order, and only when the whole domain runs dry does it cross the
 *  interconnect - walking the other domains, then each domain's threads, in coprime order again.
 *
 ...
 *  probe in @c drain_claim drives the drained-slice regime to zero overshoot, so wrapping would
 *  need @c n within a core-count of the index type's maximum.
 */
template <typename pool_type_, typename fork_type_, typename index_type_>
class invoke_distributed_for_n_dynamic {
    ...
    /** Helps every thread of @p compute_domain, in a coprime order seeded by the caller. */
  ...
  public:
    ...
    void operator()(index_type_ const thread) noexcept {
        ...
        for (auto domain = other_domains.begin(); domain != std::default_sentinel; ++domain)
            if (*domain != native_compute_domain) drain_domain_(*domain, thread, at);
    }

  private:
    /** Publishes one contiguous slice per thread in every domain. Runs before the broadcast. */
...
struct distributed_pool {
    ...
    /** Same allocator as the sub-pools: domain-placing where the kernel can honour it. */
  ...
  private:
     ...
     *  pool's @c mood_ and @c epoch_ holds a Shared copy in its own cache, and the
     *  @c threads_to_sync_ line ping-pongs between its sharers, not their directory home -
     *  per-domain placement of these signal words measured as pure noise on a 2-socket machine. The
     *  hot per-task state - every worker's @c dynamic_claim cursor inside @c pthreads_ - is
     *  node-local regardless, since each @c colocated_pool allocates it with its own domain
     *  allocator in @c spawn. Entries are sorted by compute-domain index, and the first one always
  ...
  public:
    ...
    /** Global index of the first worker in @p compute_domain; workers are numbered contiguously. */
    ...
    /** One worker's private claim cursor, drained by @c invoke_distributed_for_n_dynamic. */
...
#pragma region Core API
    ...
    /** Workers the kernel refused to place, summed over every compute domain. */
    ...
    status_t spawn( //
        ...
        return spawn(topology, topology.logical_cores_count(), exclusivity, pin_granularity);
    }
    ...
    status_t spawn( //
        ...
        for (index_t compute_domain_index = 1; compute_domain_index < colocations_count; ++compute_domain_index) {
            ...
            if (status_t const spawned = colocations[compute_domain_index].spawn(
     ...
     */
    status_t spawn_near_memory_domain( //
...
#pragma region ComputeDomains Compatibility
    ...
    /** Number of compute domains this pool spans - one pinned sub-pool each. */
    ...
    /** Returns the compute domain index owning @p global_thread_index, or the count if none. */
     ...
     *  to one node's local domains, so callbacks get the index @c compute_domain_at takes. */
 ...
 *
 *  The chase and the stream are separate experiments, so each record carries its own experiment's
 *  metric and 0 for the other; @c measured_fabric folds the log into per-metric envelopes.
 */
...
struct alignas(64) chase_slot_t {
    ...
    /** Index of the slot the walk visits next; no initializer, as @c resize_uninitialized demands
     *  trivial construction and @c thread_chase_list_ writes every slot anyway. */
...
inline void run_on_worker_(pool_type_ &pool, std::size_t const thread, work_type_ &&work) noexcept {
...
inline status_t touchers_per_position_(machine_topology_t const &topology, pool_type_ &pool,
    ...
    for (std::size_t domain = 0; domain != pool.compute_domains_count(); ++domain) {
        ...
        if (position >= touchers.size()) return status_t::config_mismatch_k; // ? A foreign topology
...
inline status_t measure_bandwidth_edges_(machine_topology_t const &topology, pool_type_ &pool, std::size_t const target,
    ...
    if (status_t const grew = stream.resize_uninitialized(words); failed(grew)) return grew;
 ...
 *      into @p levels and returning the number of distinct tiers, at least 1.
 ...
 */
inline std::size_t derive_memory_levels_(measured_edge_t const *edges, std::size_t const edges_count,
 ...
 *
 *  Two query families: edge queries @b (initiator,target) describe one interconnect link; medium
 *  queries @b (target) describe the memory pool itself, independent of any initiator.
 *
 *  Completes the @c harvest pipeline: a @c machine_topology is harvested first and stays immutable,
 *  a pool spawns on it, and the fabric then harvests through that pool's workers, snapshotting what
...
class measured_fabric {
  ...
  private:
    ...
    /** Each compute domain's @c local_memory_of, cached so @c memory_distance needs no topology. */
  ...
  public:
    ...
    /** Number of distinct derived memory tiers, at least 1. */
     ...
     */
    template <typename pool_type_>
    status_t harvest(machine_topology_t const &topology, pool_type_ &pool) noexcept {
        reset();
  ...
  private:
    template <typename pool_type_>
    status_t harvest_(machine_topology_t const &topology, pool_type_ &pool) noexcept {
        ...
        if (status_t const grew = local_memory_.resize(compute_domains); failed(grew)) return grew;
        ...
        if (status_t const grew = memory_levels_.resize(memory_domains); failed(grew)) return grew;
        compute_domains_count_ = compute_domains;
        memory_domains_count_ = memory_domains;

        dynamic_array<std::size_t> touchers;
        if (status_t const mapped = touchers_per_position_(topology, pool, touchers); failed(mapped)) return mapped;
        ...
        if (status_t const grew = evictor.resize((std::max)(evictor_bytes / sizeof(chase_slot_t), std::size_t(1)));
            failed(grew))
            return grew;
        ...
        for (std::size_t target = 0; target != touchers.size(); ++target) {
            ...
            for (std::size_t initiator = 0; initiator != pool.compute_domains_count(); ++initiator) {
                ...
                if (status_t const grew = list.resize_uninitialized(slots); failed(grew)) return grew;
                ...
                if (status_t const added = edges_.push_back(edge); failed(added)) return added;
            }
        }
        ...
        if (status_t const grew = checksums.resize(pool.threads_count()); failed(grew)) return grew;
        for (std::size_t target = 0; target != touchers.size(); ++target) {
            if (touchers[target] == unreachable_position_k) continue; // ? Nothing nearby can first-touch it
            if (status_t const measured =
                    measure_bandwidth_edges_(topology, pool, target, touchers[target], checksums, edges_);
                failed(measured))
                return measured;
        }
        ...
        if (status_t const grew = scratch.resize(memory_domains * 4); failed(grew)) return grew;
