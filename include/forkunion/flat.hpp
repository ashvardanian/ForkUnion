 ...
 *  - handle @b exceptions: you must `try-catch` them yourself and return @c void.
 ...
 *  This is handy when some logic has to be split between "performance" & "efficiency" cores,
 ...
 *    calling thread itself on @c caller_inclusive_k pools, whose slice runs inside @c unsafe_join;
 *  - the @b pollers - any threads calling @c is_complete, which is a read-only probe.
 *
 *  All synchronization is built from three cache-line-aligned atomics and plain loads, stores, and
 *  fetch-add/sub increments - no compare-and-swap chains and no mutexes on the hot path:
 *  - @c epoch_ - the generation clock: @b odd while a fork is in flight, @b even when idle;
 *    incremented once by the dispatcher on dispatch and once by the last contributor on completion,
 *    so every generation advances it by exactly two;
 *  - @c threads_to_sync_ - the countdown identifying the @b last contributor - the only thread
 *    allowed to make the completion increment;
 *  - @c mood_ - the lifecycle switch between spinning, sleeping, and exiting workers.
 *
 ...
 *  2. @b completion @b chain: every contributor decrements the countdown with @c acq_rel, chaining
 ...
 *     acquire-load observing it sees @b all contributors' results - @c is_complete included;
 ...
 *  @tparam index_type_ Use @c std::size_t, but or a smaller type for debugging.
 *  @tparam alignment_ The alignment of the thread pool. Defaults to @c default_alignment_k.
 */
template <                                                  //
...
class flat_pool {
  ...
  public:
     ...
     *  @note At the @c fu8_t and @c fu16_t debug widths a worker stalled across exactly `2^bits`
     *      epochs would alias its @c last_epoch - astronomically unlikely at @c size_t.
     */
    using epoch_index_t = index_t;

    /** Token returned from @c unsafe_for_threads; always odd. */
    ...
    struct worker_cell_t {
        ...
        /** This thread's private @ref dynamic_claim cursor for @c for_n_dynamic. */
        claim_t claim {};
    ...
    /** The pool's single buffer: one worker cell per @c alignment_k stride. */
  ...
  private:
    ...
    /** One padded cell per thread: its @c std::thread and its claim cursor. */
...
#pragma region Core API
     ...
     *  @return The thread count: 0 if uninitialized, 1 if only the main thread is used.
     *  @note This API is @b not synchronized.
     */
     ...
     *  @return `success_k` once the pool is ready; @c unsupported_k for a caller-exclusive or
    ...
    status_t spawn(                   //
        ...
        // argument - so the width the machine reports wins over the compiled guess, which stands in
        // only where no platform source names a line. Both widths are powers of two, so a cell
        ...
        // will be used in the `_worker_loop` function.
        ...
        // Initializing the thread pool can fail for all kinds of reasons, that the `std::thread`
        // documentation describes as "implementation-defined".
...
#pragma region Control Flow
    ...
    void terminate() noexcept {
        ...
        // Prepare for future spawns: a joined thread is not joinable, so destroying the cells here
        // runs `~thread` on quiescent objects rather than calling `std::terminate`.
        threads_count_ = 0;
        workers_ = {};
        _reset_fork();
...
#pragma region Indexed Task Scheduling
    ...
    /**
     *  @brief Distributes @p n similar-duration calls between threads in slices, not per index.
     ...
     *
     *  Designed for a "balanced" workload where all threads do roughly equal work, and @p fork runs
     *  @p n times, with each thread receiving a slice of consecutive tasks.
     *
...
#pragma region Advanced
     ...
     *
     *  On @c caller_inclusive_k pools this only turns @c true once @c unsafe_join runs the caller's
     *  slice, so poll-then-join applies only to @c caller_exclusive_k pools.
     */
    ...
    void unsafe_join(generation_t generation) noexcept {
        ...
        // monitored waiter arms the right line, and the completing store's release wakes it.
  ...
  private:
    ...
    void _worker_loop(thread_index_t const thread_index) noexcept {
        ...
        while (true) {
            ...
            micro_yield_t micro_yield;
            // This loop guards two lines, the epoch and the mood: a monitor arms only the hot one,
            // since a dispatch bumps `epoch_`, while the waiter's timeout bounds how late a rare
            // `mood_` change is caught.
