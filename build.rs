...
//! - `numa-memory`, `huge-pages`: force the capability on. A build that cannot honour it stops at
//!   the `#error` in `types.hpp`, with a sentence, rather than at link time with missing symbols.
...
fn main() -> Result<(), cc::Error> {
    ...
    if portable {
        assert!(
            !force_numa_memory && !force_huge_pages,
            "`portable` turns off the very capabilities `numa-memory` and `huge-pages` turn on"
        );
    ...
    if target_os == "linux" && !portable {
        ...
        } else if force_numa_memory {
            panic!("the `numa-memory` feature was requested, but `numa.h` was not found");
        }
    }
